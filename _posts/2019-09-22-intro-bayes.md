...
The names in the equation on the right is the "usual" bayesian setup, either doing inference or learning, as we shall see. 
{: .text-justify}

A generative model tries to approximate the data distribution, $$ p_{\mathcal{D}} $$, given access to the dataset. We hope that if we are able to **learn** a good generative model, we can use the learned model for **inference**. In most models we are interested in summarize all the information about the dataset $$ \mathcal{D} $$ in a finite set of parameters, thus we call them **parametric** models. So the task of learning is "discovering" the parameters of a model that would maximize the probability of $$ \mathcal{D} $$, and inference would be using the trained model to *infer* something, e.g, class value, lower dimensional representation, continuous value for a regression problem, etc.  
{: .text-justify}
...
# Maximazing the probability of the data in our model
Okay, but what exactly means to *maximize the probability* of $$ \mathcal{D} $$ in our model? Suppose our model has a set of parameters $$ \theta $$ and it models a probability distribution from the family $$ \mathcal{P} $$, which has analytical form, thus know and we can evaluate several metrics. We can setup a distribution $$p \in \mathcal{P} $$ with parameters $$ \theta $$ and evaluate the probability of our data in $$ p $$. Even thought we don't believe our data follows precisily the form of $$ \mathcal{P} $$, in real life we can accept a bit of modeling error.  
{: .text-justify}
...
Another estimate widly used is called Maximum A Posteriori (MAP), which includes a prior knowledge of $$ \mathbf{\theta} $$ when trying to find our parameters:  
{: .text-justify}
